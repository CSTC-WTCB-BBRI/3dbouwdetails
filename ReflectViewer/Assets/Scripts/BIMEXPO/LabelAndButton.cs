...
class LabelAndButton : VisualElement
{
    public new class UxmlFactory : UxmlFactory<LabelAndButton, UxmlTraits> { }

    /// <summary>
    /// Constructor. Used when creating a new LabelAndButton via scripting.
    /// </summary>
    public LabelAndButton()
    {
        ...
        this.style.width = Length.Percent(100.0f);

        // Button
        m_Button = new Button() { name = "Button" };
        m_Button.style.width = Length.Percent(50.0f);
        m_Button.style.marginLeft = 0;
        m_Button.style.marginRight = 0;
        m_Button.text = "Details";

        // Label
        m_Label = new Label() { name = "RoomLabel" };
        m_Label.style.width = Length.Percent(50.0f);
        m_Label.style.fontSize = 9;
        m_Label.style.unityTextAlign = UnityEngine.TextAnchor.MiddleLeft;

        hierarchy.Add(m_Label);
        hierarchy.Add(m_Button);
    }
    public LabelAndButton(string roomName)
    {
        this.style.flexDirection = FlexDirection.Row;
        this.style.alignContent = Align.Stretch;
        this.style.justifyContent = Justify.SpaceBetween;
        ...
        // Button
        m_Button = new Button() { name = roomName };
        m_Button.style.width = Length.Percent(40.0f);
        m_Button.style.marginLeft = 0;
        m_Button.style.marginRight = 0;
        m_Button.text = "Details";

        // Label
        m_Label = new Label() { name = roomName };
        m_Label.style.width = Length.Percent(50.0f);
    ...
    {
        UxmlStringAttributeDescription m_LabelText = new UxmlStringAttributeDescription { name = "LabelText", defaultValue = "default_value" };
        UxmlStringAttributeDescription m_ButtonText = new UxmlStringAttributeDescription { name = "ButtonText", defaultValue = "default_value" };
        ...
        {
            base.Init(ve, bag, cc);
            LabelAndButton lnb = ((LabelAndButton)ve);
            //var lnb = ve as LabelAndButton;
            //lnb.Clear();

            lnb.style.flexDirection = FlexDirection.Row;
            lnb.style.width = Length.Percent(100.0f);

            lnb.labelText = m_LabelText.GetValueFromBag(bag, cc);
            lnb.Add(new Label() { text = lnb.labelText });
            Label lab = lnb.Q<Label>();
            lab.style.width = Length.Percent(50.0f);
            lab.style.unityTextAlign = UnityEngine.TextAnchor.MiddleLeft;

            lnb.buttonText = m_ButtonText.GetValueFromBag(bag, cc);
            lnb.Add(new Button());
            Button but = lnb.Q<Button>();
            but.style.width = Length.Percent(50.0f);
            but.style.marginLeft = 0;
            but.style.marginRight = 0;
            but.text = lnb.buttonText;
        }
    }
